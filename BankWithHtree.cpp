	...
	if (!initialized) {
		...
		if (inputParameter->designTarget == CAM_chip) {
			if (inputParameter->NoPrechargeInc) {
				searchLatency = mat.subarray.bitlineDelay + mat.subarray.ColMux[mat.subarray.indexMatchline].readLatency
						+ mat.subarray.senseAmpLatency + mat.subarray.outputAcc.readLatency;

				if (inputParameter->withOutputAcc || mat.muxSenseAmp > 1) {
					cout << "[Bank] Warning: Bit serial or Mux on SA design, but latency only shows a single sense." <<endl;
				}
			} else {
				searchLatency = mat.subarray.searchLatency * mat.muxSenseAmp
						- (mat.subarray.inputBuf.readLatency) * (mat.muxSenseAmp - 1);
				if (inputParameter->withOutputAcc) {
					searchLatency *= inputParameter->wordWidth / CAM_opt.BitSerialWidth;
				}
			}
			...
			if (inputParameter->withOutputAcc) {
				searchDynamicEnergy *= inputParameter->wordWidth / CAM_opt.BitSerialWidth;
			}
